...
public class FileAdapter {
  ...
  private static Result getSourceFile(FileDatabaseAccessor db, Integer projectID, Integer fileID, String path, Integer offset, Integer length) {
    ...
    if (projectInfo.next()) {
      ...
      if (type == Project.CRAWLED) {
        ...
        String absolutepath = project.getLocation().getProjectRoot().toFile().getAbsolutePath();
       
        if (project == null) {
          return new Result("Unable to find project path " + projectPath + " for project " + projectID+" file="+path+" located at "+JavaRepositoryFactory.INPUT_REPO.toString());
        } else {
          ...
          if (contents == null) {
              return new Result("Unable to find " + path + " for " + fileID + " file="+file+" and absolute path="+absolutepath);
          } else {
            if (offset == null) {
  ...
  public static void main(String[] args) {
	  Integer fileID = 327347;
	  FileAdapter.lookupResultByFileID(fileID);
  }
}
